...
| 12 | 工程師 | CH01 與全章共用角色／Buff 模型基線 | Completed（建置）／Ready（實機） | [M5-ENG-013](M5-ENG-013-CH01共用模型基線.md) | `工程(M5)：建立共用角色模型基線` |
| 13 | 工程師 | CH01 苔冠守衛 Boss 模型 | Completed（建置）／Ready（實機） | [M5-ENG-014](M5-ENG-014-CH01Boss模型.md) | `工程(M5)：替換CH01 Boss模型` |
| 14 | 工程師 | CH01 六類模型替換、來源授權與快取更新 | Completed | [M5-ENG-015](M5-ENG-015-CH01六類模型替換.md) | `工程(M5)：替換CH01六類模型` |
| 15 | 工程師 | CH02 六類模型替換、來源授權與實機驗收 | Completed | [M5-ENG-016](M5-ENG-016-CH02六類模型替換.md) | `工程(M5)：替換CH02六類模型` |
| 16 | 工程師 | CH03 六類模型替換、來源授權與實機驗收 | Completed | [M5-ENG-017](M5-ENG-017-CH03六類模型替換.md) | `工程(M5)：替換CH03六類模型` |
| 17 | 工程師 | CH05 六類模型替換、來源授權與實機驗收 | Completed | [M5-ENG-018](M5-ENG-018-CH05六類模型替換.md) | `美術(M5)：替換CH05六類模型` |
| 18 | 工程師 | CH06 六類模型替換、來源授權與實機驗收 | Completed | [M5-ENG-019](M5-ENG-019-CH06六類模型替換.md) | `美術(M5)：替換CH06六類模型` |
| 19 | 工程師 | 弓箭手玩家模型與電擊初始距離 | Completed | [M5-ENG-020](M5-ENG-020-弓箭手玩家模型與電擊距離.md) | `工程(M5)：替換弓箭手玩家模型並調整電擊距離` |
| 20 | 工程師 | 移除玩家殘留模型 | Completed | [M5-ENG-021](M5-ENG-021-移除玩家殘留模型.md) | `工程(M5)：移除玩家殘留人物模型` |
| 21 | 工程師 | 移除第一關舊怪物模型 | Completed | [M5-ENG-022](M5-ENG-022-移除第一關舊怪物模型.md) | `工程(M5)：移除第一關舊怪物模型` |
| 22 | 工程師 | 再次修正滑鼠／觸控左右移動方向 | Completed | [M5-ENG-023](M5-ENG-023-再次修正左右移動方向.md) | `工程(M5)：再次修正左右移動方向` |

## M6：關卡節奏重整

| 順序 | 角色 | 工作包 | 狀態 | 紀錄 | Commit |
| ---: | --- | --- | --- | --- | --- |
| 1 | PM | 三輪五波、小怪生命值與回響流程設計 | Completed | [M6-PM-001](M6-PM-001-三輪五波與回響流程.md) | `PM(M6)：鎖定三輪五波與怪物血量` |
| 2 | 工程師 | 三輪五波、章內回響與 Boss 流程實作 | Completed | [M6-ENG-001](M6-ENG-001-三輪五波與章內回響實作.md) | `工程(M6)：完成三輪五波與章內回響` |
| 3 | 工程師 | 初始輸出與 Buff 傷害縮放、地面掉落寶箱 | Completed | [M6-ENG-002](M6-ENG-002-輸出與Buff傷害縮放.md) | 待本次實作提交 |
