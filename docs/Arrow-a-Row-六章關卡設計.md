...
每章固定節奏為 `5 波小怪 → 地面掉落 Buff`，重複三輪後在第 15 波結束時進入 `最終回響三選一 → Boss → Reward`。第一波在 12m，之後每 8m 一波；第 5／10 波不暫停，玩家只透過小怪掉落 Buff 強化。第 15 波後抽出 3 張不重複候選，確認後進入 Boss 前 5 秒閃爍警示，Boss 進場後靠近並停在距離玩家 3 單位。CH01 為固定教學 sequence；CH02–CH06 以同一資料 schema 配置。

目前實作中，小怪於道路可視最遠端 `z = 64` 生成，沿道路接近玩家；Boss 被擊敗後，必須等場上小怪全部死亡，或小怪跑出玩家身後（`z < -2`），才進入 Reward 結算。
