...
| `RangedShooter` | 停止蓄力 0.6s，生成速度 5m/s 的直線敵彈 | 12 | 12 | 3.5s | 教玩家橫移躲彈；敵彈命中才造成傷害。 |
| `SineFlyer` | 正弦橫移，低頻射擊 | 10 | 9 | 3.2s | 檢驗預判與射程。 |
| `Bulwark` | 前方 60% 減傷，慢速靠近 | 28 | 16 | 2.2m/s | 驗證穿透／飛劍／側向。 |

敵人傷害只在 telegraph 後發生。遠程敵人在蓄力期間停止前進，完成蓄力後生成沿自身 X 軸直線前進、速度 `5m/s` 的敵彈；敵彈必須碰到玩家才造成傷害，命中後立即回收。接觸傷害對同一敵人有 1.0 秒玩家受擊 CD。所有敵人有 `threatScore` 供索敵同距離排序。
