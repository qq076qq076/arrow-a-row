...
export interface M1RunSnapshot {
  ...
  readonly boss: BossSnapshot | undefined; readonly bossWarningSeconds: number; readonly rewardOptions: readonly RewardId[]; readonly selectedReward: RewardId | undefined; readonly earnedGold: number;
}
...
export const BOSS_START_DISTANCE = 86;
export const BOSS_WARNING_SECONDS = 5;
export const BOSS_STOP_DISTANCE = 3;
export const BOSS_WARNING_START_DISTANCE = BOSS_START_DISTANCE - BOSS_WARNING_SECONDS * WORLD_SPEED;
...
export class M1RunSimulation {
  ...
  private player = { x: 0, hp: 100, maxHp: 100, damage: BASE_ARROW_DAMAGE, projectileCount: 1, arrowSpeed: 24, pierceCount: 0, movementSpeed: PLAYER_MOVE_SPEED, damageReduction: 0, fireRateMultiplier: 1, arrowCharge: 0, pierceCharge: 0, lightningTargetCount: 2, lightningDamagePerSecond: BASE_LIGHTNING_DAMAGE_PER_SECOND, lightningRange: 1, lightningTargetCharge: 0, enemyCountMultiplier: 1 };
  private attackCooldownSeconds = 0; private attackIntervalSeconds = 0.45; private lightningCooldownSeconds = 0; private nextArrowId = 1; private nextEffectId = 1; private earnedGold = 0; private collectedShards = 0; private bossWarningSeconds = 0; private boss: MutableBoss | undefined;
  private selectedReward: RewardId | undefined; private rewardOptions: RewardId[] = []; private randomState = 1; private runNumber = 0; private readonly selectedGateIds = new Set<string>(); private readonly spawnedWaveIds = new Set<string>(); private readonly positionedEnemyIds = new Set<string>(); private readonly swarmDuplicatedIds = new Set<string>(); private readonly enemies: MutableEnemy[] = []; private readonly arrows: MutableArrow[] = []; private readonly hits: MutableHit[] = []; private readonly pickups: MutablePickup[] = []; private lightningTargetIds: string[] = [];
  ...
  public start(modifiers: RunModifiers = {}, chapterId: ChapterId = 'ch01_meadow'): void {
    ...
    this.phase = 'playing'; this.elapsedSeconds = 0; this.distanceMeters = 0; this.targetX = 0; this.earnedGold = 0; this.bossWarningSeconds = 0; this.boss = undefined; this.selectedReward = undefined; this.rewardOptions = [];
    const maxHp = 100 + (modifiers.healthLevel ?? 0) * 10;
    this.player = { x: 0, hp: maxHp, maxHp, damage: BASE_ARROW_DAMAGE * (1 + (modifiers.damageLevel ?? 0) * 0.2), projectileCount: 1, arrowSpeed: 24 + (modifiers.arrowSpeedLevel ?? 0) * 2, pierceCount: modifiers.pierceLevel ?? 0, movementSpeed: PLAYER_MOVE_SPEED + (modifiers.movementLevel ?? 0), damageReduction: 0, fireRateMultiplier: 1, arrowCharge: 0, pierceCharge: 0, lightningTargetCount: 2, lightningDamagePerSecond: BASE_LIGHTNING_DAMAGE_PER_SECOND, lightningRange: 1, lightningTargetCharge: 0, enemyCountMultiplier: 1 }; this.attackCooldownSeconds = 0; this.attackIntervalSeconds = Math.max(0.25, 0.45 - (modifiers.fireRateLevel ?? 0) * 0.04); this.lightningCooldownSeconds = 0; this.nextArrowId = 1; this.nextEffectId = 1; this.collectedShards = 0;
    this.selectedGateIds.clear(); this.spawnedWaveIds.clear(); this.positionedEnemyIds.clear(); this.swarmDuplicatedIds.clear(); this.enemies.length = 0; this.arrows.length = 0; this.hits.length = 0; this.pickups.length = 0; this.lightningTargetIds = [];
  ...
  public continueToNextChapter(): boolean {
    ...
    this.chapterId = next.id; this.phase = 'playing'; this.elapsedSeconds = 0; this.distanceMeters = 0; this.targetX = 0; this.bossWarningSeconds = 0; this.boss = undefined; this.selectedReward = undefined; this.rewardOptions = []; this.earnedGold = 0; this.attackCooldownSeconds = 0;
    this.enemies.length = 0; this.arrows.length = 0; this.hits.length = 0; this.pickups.length = 0; this.lightningTargetIds = []; this.selectedGateIds.clear(); this.spawnedWaveIds.clear(); this.positionedEnemyIds.clear(); this.swarmDuplicatedIds.clear();
  ...
  public tick(deltaSeconds: number): void {
    if (this.phase !== 'playing') return;
    this.elapsedSeconds += deltaSeconds; this.movePlayer(deltaSeconds); this.resolveGates(); this.spawnEnemies(); this.spawnExtraWaves(); this.duplicateSwarmEnemies(); this.updateEnemies(deltaSeconds);
    if (this.boss === undefined) {
      ...
      } else if (this.distanceMeters >= BOSS_WARNING_START_DISTANCE) {
        this.bossWarningSeconds = BOSS_WARNING_SECONDS;
      }
    }
  ...
  private tryArrowHit(arrow: MutableArrow): boolean { const enemy = this.enemies.filter((candidate) => !arrow.hitEnemyIds.includes(candidate.id) && candidate.deathSeconds <= 0 && Math.abs(candidate.z - arrow.z) < 0.75 && Math.abs(candidate.x - arrow.x) < 0.85).sort((left, right) => left.z - right.z)[0]; if (enemy !== undefined) { arrow.hitEnemyIds.push(enemy.id); this.damageEnemy(enemy, this.player.damage * getArrowDamageMultiplier(arrow.z)); return true; } const boss = this.boss; if (boss !== undefined && !arrow.hitBoss && !boss.isDefeated && Math.abs(boss.z - arrow.z) < 1 && Math.abs(arrow.x) < 1.5) { arrow.hitBoss = true; this.damageBoss(boss, this.player.damage * getArrowDamageMultiplier(arrow.z)); return true; } return false; }
