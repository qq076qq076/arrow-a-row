...
export class M1RunSimulation {
  ...
  private spawnBoss(): void { const hp = getChapterDefinition(this.chapterId).bossHp; const isMirrorChapter = this.chapterId === 'ch02_viaduct'; const isForgeChapter = this.chapterId === 'ch03_forge'; this.enemies.length = 0; this.boss = { hp, maxHp: hp, phase: 1, telegraphSeconds: 0.8, telegraphText: isForgeChapter ? '熔脈監工正在蓄積震波！' : isMirrorChapter ? '鏡潮校準者正在鎖定航線！' : '苔冠守衛被靜滯困住了。', attackCooldownSeconds: 2.5, isDefeated: false }; }
