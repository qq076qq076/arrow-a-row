...
// Keep the old z=38-to-player travel time while moving the visible spawn point to the far road edge.
const ENEMY_APPROACH_SPEED = WORLD_SPEED * ENEMY_SPAWN_Z / 38;
...
export class M1RunSimulation {
  ...
  public continueToNextChapter(): boolean {
    ...
    this.chapterId = next.id; this.phase = 'playing'; this.elapsedSeconds = 0; this.distanceMeters = 0; this.targetX = 0; this.bossWarningSeconds = 0; this.boss = undefined; this.selectedReward = undefined; this.rewardOptions = []; this.wavesCompleted = 0; this.echoRound = 0; this.earnedGold = 0; this.attackCooldownSeconds = 0;
  ...
  private updateEnemies(deltaSeconds: number): void { for (const enemy of this.enemies) { if (!this.positionedEnemyIds.has(enemy.id)) { enemy.x = Math.max(-4.6, Math.min(4.6, enemy.x + (this.nextRandomIndex(17) - 8) / 10)); this.positionedEnemyIds.add(enemy.id); } if (enemy.deathSeconds > 0) { enemy.deathSeconds = Math.max(0, enemy.deathSeconds - deltaSeconds); continue; } enemy.z -= ENEMY_APPROACH_SPEED * deltaSeconds; if (enemy.hp > 0 && enemy.z <= 1.2 && Math.abs(this.player.x - enemy.x) < 1.2) { this.resolveEnemyCollision(enemy); continue; } enemy.attackCooldownSeconds -= deltaSeconds; if (enemy.kind === 'ranged') { if (enemy.attackCooldownSeconds <= 0 && enemy.telegraphSeconds <= 0) enemy.telegraphSeconds = 0.6; if (enemy.telegraphSeconds > 0) { enemy.telegraphSeconds -= deltaSeconds; if (enemy.telegraphSeconds <= 0) { if (Math.abs(this.player.x - enemy.x) < 1.2) this.takeDamage(12); enemy.attackCooldownSeconds = 3.5; } } } } }
  ...
  private updatePickups(deltaSeconds: number): void { for (let index = this.pickups.length - 1; index >= 0; index -= 1) { const pickup = this.pickups[index]; if (pickup === undefined) continue; pickup.z -= WORLD_SPEED * deltaSeconds; if (pickup.z < 1.4 && Math.abs(pickup.x - this.player.x) < 1.5) { this.collectPickup(pickup); this.pickups.splice(index, 1); } else if (pickup.z < -2) this.pickups.splice(index, 1); } }
