...
export interface EnemyProjectileSnapshot { readonly id: number; readonly x: number; readonly z: number; readonly vx: number; readonly vz: number; readonly damage: number; }
...
export interface M1RunSnapshot {
  ...
  readonly enemies: readonly EnemySnapshot[]; readonly gates: readonly GateSnapshot[]; readonly arrows: readonly ArrowSnapshot[]; readonly enemyProjectiles: readonly EnemyProjectileSnapshot[]; readonly hits: readonly HitSnapshot[]; readonly pickups: readonly PickupSnapshot[]; readonly lightningTargetIds: readonly string[]; readonly collectedShards: number; readonly selectedGateIds: readonly string[];
...
interface MutableEnemyProjectile { id: number; x: number; z: number; vx: number; vz: number; damage: number; }
...
export class M1RunSimulation {
  ...
  private player = { x: 0, hp: 100, maxHp: 100, damage: BASE_ARROW_DAMAGE, projectileCount: 0, arrowSpeed: 24, pierceCount: 0, movementSpeed: PLAYER_MOVE_SPEED, damageReduction: 0, lifeSteal: 0, fireRateMultiplier: 1, arrowCharge: 0, pierceCharge: 0, lightningTargetCount: 2, lightningDamagePerSecond: BASE_LIGHTNING_DAMAGE_PER_SECOND, lightningRange: 3, lightningTargetCharge: 0, cannonUnlocked: false, cannonDamage: BASE_CANNON_DAMAGE, cannonBlastRadius: CANNON_BLAST_RADIUS, cannonFireRateMultiplier: 1, enemyCountMultiplier: 1 };
  private attackCooldownSeconds = 0; private attackIntervalSeconds = 0.45; private cannonCooldownSeconds = 0; private lightningCooldownSeconds = 0; private nextArrowId = 1; private nextEnemyProjectileId = 1; private nextEffectId = 1; private earnedGold = 0; private collectedShards = 0; private bossWarningSeconds = 0; private boss: MutableBoss | undefined;
  private selectedReward: RewardId | undefined; private rewardOptions: RewardId[] = []; private wavesCompleted = 0; private echoRound = 0; private randomState = 1; private runNumber = 0; private readonly selectedGateIds = new Set<string>(); private readonly positionedEnemyIds = new Set<string>(); private readonly swarmDuplicatedIds = new Set<string>(); private readonly enemies: MutableEnemy[] = []; private readonly arrows: MutableArrow[] = []; private readonly enemyProjectiles: MutableEnemyProjectile[] = []; private readonly hits: MutableHit[] = []; private readonly pickups: MutablePickup[] = []; private lightningTargetIds: string[] = [];
  ...
  public continueToNextChapter(): boolean {
    ...
    this.enemies.length = 0; this.arrows.length = 0; this.enemyProjectiles.length = 0; this.hits.length = 0; this.pickups.length = 0; this.lightningTargetIds = []; this.selectedGateIds.clear(); this.positionedEnemyIds.clear(); this.swarmDuplicatedIds.clear();
  ...
  public tick(deltaSeconds: number): void {
    if (this.phase !== 'playing') return;
    this.elapsedSeconds += deltaSeconds; this.movePlayer(deltaSeconds); this.resolveGates(); this.spawnEncounterWave(); this.duplicateSwarmEnemies(); this.updateEnemies(deltaSeconds); this.updateEnemyProjectiles(deltaSeconds);
  ...
  private createGates(): MutableGate[] {
    ...
    const second = drawPair(BUFF_IDS); const third = drawPair(BUFF_IDS);
  ...
  private drawPickupBuffId(): BuffId { return BUFF_IDS[this.nextRandomIndex(BUFF_IDS.length)]!; }
  ...
  private resolveEnemyCollision(enemy: MutableEnemy): void { const collisionDamage = enemy.hp; this.player.hp -= collisionDamage; const pickup = this.damageEnemy(enemy, collisionDamage); if (pickup !== undefined) { this.collectPickup(pickup); this.pickups.splice(this.pickups.indexOf(pickup), 1); } }
  private damageEnemy(enemy: MutableEnemy, damage: number): MutablePickup | undefined { if (enemy.hp <= 0 || enemy.deathSeconds > 0) return undefined; const actualDamage = Math.min(enemy.hp, Math.max(0, damage)); if (actualDamage <= 0) return undefined; enemy.hp -= actualDamage; this.healFromDamage(actualDamage); this.addHit(enemy.x, enemy.z); if (enemy.hp <= 0) { enemy.hp = 0; enemy.deathSeconds = 0.45; const buffId = this.drawPickupBuffId(); const pickup = { id: this.nextEffectId++, x: enemy.x, z: enemy.z, buffId, label: BUFF_CATALOG[buffId].pickupLabel }; this.pickups.push(pickup); return pickup; } return undefined; }
