...
export interface ArrowSnapshot { readonly id: number; readonly x: number; readonly z: number; readonly vx: number; }
...
interface MutableArrow { id: number; x: number; z: number; vx: number; }
...
export class M1RunSimulation {
  ...
  public restore(snapshot: M1RunSnapshot): boolean {
    ...
    this.player = { ...snapshot.player }; this.attackCooldownSeconds = 0.1; this.nextArrowId = Math.max(1, ...snapshot.arrows.map((arrow) => arrow.id + 1)); this.earnedGold = snapshot.earnedGold; this.collectedShards = snapshot.collectedShards ?? 0; this.selectedReward = snapshot.selectedReward; this.rewardOptions = [...snapshot.rewardOptions];
    this.selectedGateIds.clear(); snapshot.selectedGateIds.forEach((id) => this.selectedGateIds.add(id)); this.positionedEnemyIds.clear(); snapshot.enemies.forEach((enemy) => this.positionedEnemyIds.add(enemy.id)); this.enemies.splice(0, this.enemies.length, ...snapshot.enemies.map((enemy) => ({ ...enemy, deathSeconds: enemy.deathSeconds ?? 0, attackCooldownSeconds: 0.5 }))); this.arrows.splice(0, this.arrows.length, ...snapshot.arrows.map((arrow) => ({ ...arrow, vx: arrow.vx ?? 0 })));
  ...
  private updateArrows(deltaSeconds: number): void { for (const arrow of this.arrows) { arrow.x += arrow.vx * deltaSeconds; arrow.z += Math.sqrt(Math.max(0, this.player.arrowSpeed ** 2 - arrow.vx ** 2)) * deltaSeconds; } for (let index = this.arrows.length - 1; index >= 0; index -= 1) { const arrow = this.arrows[index]; if (arrow !== undefined && (arrow.z > 45 || Math.abs(arrow.x) > 12)) this.arrows.splice(index, 1); } for (let index = this.enemies.length - 1; index >= 0; index -= 1) { const enemy = this.enemies[index]; if (enemy !== undefined && (enemy.z < -2 || (enemy.hp <= 0 && enemy.deathSeconds <= 0))) this.enemies.splice(index, 1); } }
