...
export class M1RunSimulation {
  ...
  private fireAtNearestTarget(): void { this.attackCooldownSeconds = this.attackIntervalSeconds * this.player.fireRateMultiplier; const enemy = this.enemies.filter((candidate) => candidate.z > 0 && candidate.deathSeconds <= 0).sort((left, right) => left.z - right.z)[0]; const targetX = this.boss !== undefined && !this.boss.isDefeated ? 0 : enemy?.x; const targetZ = this.boss !== undefined && !this.boss.isDefeated ? this.boss.z : enemy?.z; const aimAngle = targetX === undefined || targetZ === undefined ? 0 : Math.atan2(targetX - this.player.x, Math.max(1, targetZ - 1)); for (let index = 0; index < this.player.projectileCount; index += 1) { const spreadIndex = index - (this.player.projectileCount - 1) / 2; const angleRadians = aimAngle + spreadIndex * 0.14; const offset = spreadIndex * 0.18; this.arrows.push({ id: this.nextArrowId++, x: this.player.x + offset, z: 1, vx: Math.sin(angleRadians) * this.player.arrowSpeed, piercesRemaining: this.player.pierceCount, hitEnemyIds: [], hitBoss: false }); } }
