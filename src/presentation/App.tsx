...
export function App(): ReactElement {
  const canvasContainerRef = useRef<HTMLDivElement>(null); const simulationRef = useRef(new M1RunSimulation()); const checkpointRef = useRef(new RunCheckpointRepository()); const profileRef = useRef(new ProfileRepository());
  const [snapshot, setSnapshot] = useState<M1RunSnapshot>(INITIAL_SNAPSHOT); const [profile, setProfile] = useState<Profile>(DEFAULT_PROFILE); const [screen, setScreen] = useState<'game' | 'shop'>('game'); const [selectedCard, setSelectedCard] = useState<RewardId>(); const [isWebGlSupported] = useState(() => document.createElement('canvas').getContext('webgl2') !== null);

  useEffect(() => { void profileRef.current.loadAsync().then(setProfile).catch(() => undefined); }, []);
  useEffect(() => {
  ...
  const startRun = (): void => { setScreen('game'); setSelectedCard(undefined); simulationRef.current.start(); setSnapshot(simulationRef.current.snapshot()); };
  ...
  return <main className="app-shell"><div ref={canvasContainerRef} className="game-canvas" aria-hidden="true" onPointerDown={movePlayer} onPointerMove={movePlayer} />
    {screen === 'shop' ? <Shop profile={profile} onBuy={buy} onBack={() => setScreen('game')} /> : <>
      ...
      {snapshot.phase === 'menu' && <MainMenu onStart={startRun} onShop={() => setScreen('shop')} gold={profile.gold} />}
...
function MainMenu({ onStart, onShop, gold }: { readonly onStart: () => void; readonly onShop: () => void; readonly gold: number }): ReactElement { return <section className="main-menu" aria-label="主選單"><p className="eyebrow">CH01 垂直切片</p><h1>Arrow a Row</h1><p>單手拖曳，選擇路徑，自動射擊。</p><button type="button" onClick={onStart}>開始第一章</button><button type="button" className="secondary" onClick={onShop}>永久強化｜{gold} 金幣</button></section>; }
