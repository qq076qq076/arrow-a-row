...
export function App(): ReactElement {
  ...
  const startRun = (): void => { setScreen('game'); setSelectedCard(undefined); setHasCheckpoint(false); simulationRef.current.start(profile); setSnapshot(simulationRef.current.snapshot()); };
...
function RunHud({ snapshot }: { readonly snapshot: M1RunSnapshot }): ReactElement { return <><header className="hud"><p>HP {Math.ceil(snapshot.player.hp)} / {snapshot.player.maxHp}</p><p>{snapshot.player.projectileCount} 箭｜晶塵 {snapshot.collectedShards}</p></header>{snapshot.boss !== undefined ? <section className="boss-hud"><strong>苔冠守衛｜P{snapshot.boss.phase}</strong><progress value={snapshot.boss.hp} max={snapshot.boss.maxHp} />{snapshot.boss.telegraphSeconds > 0 && <p>{snapshot.boss.telegraphText}</p>}</section> : <p className="run-progress">距離 {Math.floor(snapshot.distanceMeters)}m / Boss</p>}</>; }
